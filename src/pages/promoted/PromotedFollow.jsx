...
export default function PromotedFollow() {
  const [tasks, setTasks] = useState([]);

  useEffect(() => {
    (async () => {
      try {
        const res = await api.get("/tasks/promoted/follow");
        setTasks(res.data || []);
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      <h2 className="text-2xl font-bold mb-4">Promoted follow Tasks</h2>
      {tasks.length ? (
        tasks.map(t => (
        ...
        <p>No promoted follow tasks available yet.</p>
      )}
    </div>
  );
}
