...
export default function PromotedComment() {
  const [tasks, setTasks] = useState([]);

  useEffect(() => {
    (async () => {
      try {
        const res = await api.get("/tasks/promoted/comment");
        setTasks(res.data || []);
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      <h2 className="text-2xl font-bold mb-4">Promoted comment Tasks</h2>
      {tasks.length ? (
        tasks.map(t => (
        ...
        <p>No promoted comment tasks available yet.</p>
      )}
    </div>
  );
}
