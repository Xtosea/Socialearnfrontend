...
import { getPromotionCosts, getPromotedCounts, getPromotedTasks } from "../api/promotion"; // ✅ fixed import
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {PROMOTED_CARDS.map(card => {
              const link = card.type === "watch" 
                ? `/promoted/watch/${card.platform}` 
                : `/promoted/action/${card.platform}`;

              return (
                <Link key={card.platform + card.type} to={link} className={`block p-4 rounded shadow text-white ${card.color} hover:opacity-90 transition`}>
