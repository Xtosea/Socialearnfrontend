...
export default function Sidebar({ closeMenu }) {
  ...
  // Hide dashboard/tasks/admin menu on Contact page
  const hideMenuItems = location.pathname === "/contact";
  ...
  return (
    <div className="flex flex-col h-full bg-white shadow-md p-4">
      ...
      <nav className="flex-1 space-y-3 overflow-y-auto">
        {/* Only show dashboard/tasks/admin if NOT on Contact page */}
        {!hideMenuItems && user && (
          <>
            <Link to="/dashboard" onClick={closeMenu} className={`flex items-center gap-2 p-2 ${isActive("/dashboard")}`}>
...
>
  <Video className="w-4 h-4 text-green-500" /> Social Feed
            ...
            <Link to="/wallet" onClick={closeMenu} className={`flex items-center gap-2 p-2 ${isActive("/wallet")}`}>
              <Wallet className="w-4 h-4" /> Wallet
            </Link>

            <Link to="/history" onClick={closeMenu} className={`flex items-center gap-2 p-2 ${isActive("/history")}`}>
              <History className="w-4 h-4" /> History
            </Link>

            <Link to="/leaderboard" onClick={closeMenu} className={`flex items-center gap-2 p-2 ${isActive("/leaderboard")}`}>
              <Trophy className="w-4 h-4" /> Leaderboard
            </Link>

            {/* Promote Videos Dropdown */}
            <button onClick={() => setOpenVideos(!openVideos)} className="flex items-center justify-between w-full hover:text-blue-600">
            ...
            {openVideos && (
              <div className="ml-6 space-y-2">
                <button onClick={() => setOpenVideos(false)} className="flex items-center gap-2 text-gray-600 hover:text-blue-600">
            ...
            <button onClick={() => setOpenEngagements(!openEngagements)} className="flex items-center justify-between w-full hover:text-blue-600">
              <span className="flex items-center gap-2">
            ...
            {openEngagements && (
              <div className="ml-6 space-y-2">
                <button onClick={() => setOpenEngagements(false)} className="flex items-center gap-2 text-gray-600 hover:text-blue-600">
                  <ArrowLeft className="w-4 h-4" /> Go Back
                </button>
            ...
            {/* Admin Panel */}
            <Link to="/admin" onClick={closeMenu} className={`flex items-center gap-2 p-2 ${isActive("/admin")}`}>
              <Shield className="w-4 h-4" /> Admin Panel
            </Link>
          </>
        )}

        {/* Contact link only visible to logged-in users */}
        {user && (
          <Link to="/contact" onClick={closeMenu} className={`flex items-center gap-2 p-2 ${isActive("/contact")}`}>
